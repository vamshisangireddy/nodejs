...
        }
        
        // NOTE: Infrastructure stages are often run in a separate, dedicated pipeline.
        // Included here for a complete single-file view of the process.
        stage('🏗️ Plan Infrastructure (Terraform)') {
            steps {
                dir('infrastructure/terraform') {
                    sh 'terraform init'
                    sh 'terraform plan -out=tfplan'
                }
                input 'Does the Terraform plan look correct?'
            }
        }
